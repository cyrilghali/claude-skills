...
3. **One why.** A single sentence with the reason that changes implementation or reading. The trail (quotes, corroborations, "vérifié dans le code", the facts that convinced the author) stays in the reply to the requester.
...
4. Cold read, when the text is longer than about 300 words: spawn a fresh subagent with no context (a mid-size model is enough), told it is a colleague new to the subject, asked to list only what still blocks it (heading without stake, undefined term, missing why, filler, contradiction). Fix every item that is a real blocker. The text is publishable only after this pass.
...
A PR body is one sentence of intent; what was checked goes to the author in the reply, in this register. A pedagogical document for a newcomer keeps its own structure (context before jargon, a running example) with this register inside it. A three-line note stays three lines: no heading, no table.

## Evals

Write your own cases from texts of your project: each with a named trap, judged with a 0-2 rubric per dimension by a fresh strict subagent, and run through `hooks/prose-gate.sh doc` on a real file path (the gate reads the file from disk).
